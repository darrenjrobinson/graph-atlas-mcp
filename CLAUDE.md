...
| `today` is computed independently in the collector and the release script. A UTC-midnight crossing between them desynchronises the tag date from `snapshot_date`, which empties the release notes and makes clients re-download forever | `scripts/collect-and-diff.js`, `scripts/create-release.js` |
